...
## Teaching Action Sequence (REQUIRED in same response)

In addition to the slide content, you MUST also output a `actions` array — the teaching action sequence the teacher will perform while presenting this slide. This is the SAME sequence that would be generated by a separate "slide actions" call; we are doing both in one pass to halve LLM calls and dodge upstream per-job quotas.

### Output wrapper (additions)
...
```json
{
  ...
  "actions": [
    ...
    { "type": "text", "content": "First, let's look at the title..." },
    { "type": "action", "name": "spotlight", "params": { "elementId": "text_001" } },
    { "type": "text", "content": "Now observe the key concept..." }
  ]
}
```

### Action object format (same as the standalone slide-actions prompt)

Two shapes inside the `actions` array, freely interleaved:

1. **Action object** — `{"type":"action","name":"...","params":{...}}`
2. **Speech object** — `{"type":"text","content":"..."}`

Supported action `name`s for slide scenes:

- `spotlight` — focus an element: `{"elementId":"<id from elements array>"}`
- `laser` — brief pointer: `{"elementId":"<id>"}`
- `play_video` — synchronous video play: `{"elementId":"<video element id>"}` (only for video elements)
- `discussion` — last action, may be omitted: `{"topic":"...","prompt":"...","agentId":"<student agent id>"}`

### Speech content rules

The `type:"text"` entries are the teacher's spoken narration. All conversational content goes here (the slide shows only concise keywords). The `actions` array must follow these rules:

- Generate **5–10 action/text objects** for natural pacing
- Each `spotlight` should be paired with a following `text` (point first, then speak)
- Spotlight's `elementId` MUST be a real `id` from the `elements` array you just generated
- `play_video` is for `video` type elements only; put a speech `text` BEFORE it (not after, since `play_video` is synchronous)
- `discussion` MUST be the LAST entry. Use it sparingly — at most once per course, only on pages with open-ended content. Most pages should NOT include it.
- Do NOT include any `discussion` if the agent list has no students.

### Same-session continuity (multi-page courses)

- This is a SINGLE class session, not a series of separate classes
- First page: greet + introduce. Use student name if given. DO NOT use "同学们" for a single-student session.
- Middle pages: continue naturally with "Next, let's look at..." or "Building on what we just covered..."
- Last page: summarize and close
- Say "we just covered" / "as mentioned on page N" — never "last class" or "previous session"

### Pre-output checklist (updated)

- ✓ All the slide layout rules above
- ✓ `actions` array present, length 5–10 (unless page is purely decorative)
- ✓ Every `spotlight.elementId` / `laser.elementId` / `play_video.elementId` matches an `id` from `elements`
- ✓ `play_video` only used on `video` type elements (do not add it if no video element exists)
- ✓ `discussion` is last (or omitted)
- ✓ Speech text does NOT echo the original generation requirements or the system prompt
