...
function mapDbWork(w: DbWork, index: number): Work {
  ...
  return {
    ...
    // 「精选」身份只靠右上角角标 + 置顶排序体现。
    className: VISUAL_CLASSES[index % VISUAL_CLASSES.length],
...
export default function WorksPage() {
  ...
  return (
    <div className="works-page">
      ...
      {/* 精选作品不单独成区 —— 保持便签墙这一个统一的视觉语言。
          精选只做两件事：① 在便签墙里置顶；② 卡片右上角挂一个「★ 精选」小角标。
          之前试过给精选做专属深蓝卡片专区，风格与贴纸便签不一致，已回退。 */}
        ...
        {!loading && !loadError && visibleWorks.length > 0 ? (
          <header className="works-wall-head">
            ...
            <p className="works-wall-count">
              ...
              {featuredWorks.length > 0 && !query.trim()
                ? ` · 其中 ${featuredWorks.length} 个精选`
                : ''}
