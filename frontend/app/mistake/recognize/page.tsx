...
export default function MistakeRecognizePage() {
  ...
  const ocrFailureHint =
    showFailure && pending?.rawModelText?.includes('[ocr-error]')
      ? pending.rawModelText.replace(/^\s*\[ocr-error\]\s*/i, '').trim()
      : '';
      ...
      <div className="grid gap-6 md:grid-cols-[280px_1fr] items-start">
        <Card className="overflow-hidden">
          {imageUrl || pending?.imageUrl ? (
            <img alt="识别确认题目图片" className="w-full object-contain bg-muted/30 max-h-96" src={imageUrl || pending?.imageUrl} />
          ) : (
        ...
        <Card className="grid gap-4 p-6">
          {showFailure ? (
            <>
              ...
              {!error && ocrFailureHint ? (
                <p className="text-sm text-destructive">{ocrFailureHint}</p>
              ) : null}
            </>
          ) : (
            <>
