...
import {
  normalizeLatexSourceForRendering,
  sanitizeRawLatexInTextElements,
...
describe('sanitizeRawLatexInTextElements', () => {
  ...
  it('repairs latex commands that were corrupted by JSON escape parsing', () => {
    const repaired = normalizeLatexSourceForRendering('x = \frac{7}{24} + \theta');

    expect(repaired).toBe(String.raw`x = \frac{7}{24} + \theta`);
  });
});
