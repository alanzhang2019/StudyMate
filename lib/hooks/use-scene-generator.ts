...
export async function generateAndStoreTTS(
  ...
  // Use profile specific voice if provided, otherwise fallback to global setting
  const effectiveVoice = profileTtsVoice || settings.ttsVoice;
  
  const providerOptions =
    settings.ttsProviderId === 'voxcpm-tts'
      ? {
          ...(ttsProviderConfig?.providerOptions || {}),
          ...(await getVoxCPMProviderOptions(effectiveVoice, { role: 'teacher', language })),
        }
      : undefined;
  const response = await fetch('/api/generate/tts', {
    ...
    body: JSON.stringify({
      ...
      profileTtsVoice, // Send to backend so it knows this is an explicit override
    }),
    signal,
  });
